...
describe AppMarketScraper do
  ...
  it 'do SearchScraper class validator' do
    ...
    while AppMarketScraper::Play.array.size == 0
      AppMarketScraper::Play.array.elements.each_with_index do |value, index|
        AppMarketScraper.threads.add(Thread.new {
            # puts value
            AppMarketScraper::Play.array.elements.delete(index)
            AppMarketScraper::Play::Detail::Scraper.new(value.package).start
        })
      end
      AppMarketScraper.threads.elements.each do|thread|
        thread.join
    ...
    # AppMarketScraper.threads.elements.clear
